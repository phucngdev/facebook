...
var list_avt_watch = document.querySelectorAll(".list_left_body_item_avt");

var colors = ["#1977f2", "#f02849", "#e1c8be", "#2abaa6", "#f7b828"];

for (var i = 0; i < listItems.length; i++) {
  
  
  // Gán một màu nền khác nhau cho mỗi phần tử con
  listItems[0].style.backgroundColor = "#3a3b3c";
  var childItems = listItems[i].querySelectorAll(".list_left_body_item_avt");
  childItems[0].style.backgroundColor = colors[(i+j) % colors.length];
  for (var j = 0; j < childItems.length; j++) {
    childItems[j].style.backgroundColor = colors[(i+j) % colors.length];
  }
  
  // Gán sự kiện click cho mỗi phần tử
  listItems[i].addEventListener("click", function() {
    
    // Thay đổi màu nền của phần tử được click và các phần tử con của nó
    var clickedChildItems = this.querySelectorAll(".list_left_body_item_avt");
    for (var k = 0; k < listItems.length; k++) {
      if (listItems[k] === this || listItems[k] === this.parentNode) {
        listItems[k].style.backgroundColor = "#3a3b3c";
      } else {
        listItems[k].style.backgroundColor = "";
      }
    }
    // for (var l = 0; l < clickedChildItems.length; l++) {
    //   var index = Array.from(clickedChildItems).indexOf(this);
    //   this.style.backgroundColor = colors[index];
      
    //   // clickedChildItems[l].style.backgroundColor = colors[(Array.from(this.parentNode.children).indexOf(this) + l) % colors.length];
    // }
